...
export default function ScannerPage() {
    ...
    const exportToCSV = () => {
        ...
        link.setAttribute("download", `init_attendance_${new Date().toISOString().split("T")[0]}.csv`);
        link.click();
    };
        ...
        <main className="relative flex min-h-screen flex-col items-center bg-black p-4 text-white md:p-8">
            ...
            {modalError && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md">
                    <div className="mx-4 w-full max-w-sm animate-fade-in rounded-2xl border border-white/20 bg-black p-8 text-center shadow-[0_0_50px_rgba(255,255,255,0.05)]">
                        ...
                        <h3 className="mb-2 text-xl font-bold text-white">Duplicate Found</h3>
                        <p className="text-white/70">{modalError}</p>
                        <button onClick={() => setModalError(null)} className="mt-6 w-full rounded-xl bg-white/10 py-3 text-sm font-semibold text-white hover:bg-white/15 transition-colors">Close</button>
                    </div>
                </div>
            )}
            ...
            <div className="z-10 w-full max-w-4xl">
                <div className="mb-8 flex items-center justify-between">
                    ...
                    <div className="text-center">
                        <h1 className="text-xl font-bold tracking-tight md:text-2xl">Scanner</h1>
                        <div className="flex items-center justify-center gap-2 text-[10px] text-white/30 uppercase tracking-widest">
                            <span className="h-1 w-1 bg-white/50 rounded-full animate-pulse" />
                            Supabase Connected
                        </div>
                    </div>
                    <button onClick={clearAll} className="px-3 py-1 text-xs font-semibold text-white/50 bg-white/5 rounded-lg hover:bg-white/10 hover:text-white transition-colors">Clear All</button>
                </div>
                ...
                <div className="grid gap-8 lg:grid-cols-2">
                    <div className="space-y-6">
                        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] p-6 backdrop-blur-xl shadow-xl hover:border-white/20 transition-all duration-500">
                            ...
                            {!isScanning ? (
                                <div className="flex flex-col items-center justify-center rounded-xl bg-black/40 py-20 text-center ring-1 ring-white/5">
                                    <button onClick={startScanner} className="group relative rounded-xl bg-white px-10 py-4 text-xs font-black uppercase tracking-widest text-black shadow-[0_0_20px_rgba(255,255,255,0.08)] hover:shadow-[0_0_30px_rgba(255,255,255,0.15)] transition-all">
                                        <span className="relative z-10">Launch Camera</span>
                                        <div className="absolute inset-x-0 bottom-0 h-1 bg-black/20 scale-x-0 group-hover:scale-x-100 transition-transform origin-left" />
                                    </button>
                                </div>
                            ) : (
                                <button onClick={stopScanner} className="mt-4 w-full rounded-xl border border-white/10 bg-white/5 py-4 text-xs font-black uppercase tracking-widest text-white/60 hover:bg-white/10 hover:text-white transition-colors">Stop Camera</button>
                            )}
                        </div>
                        ...
                        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] p-6 backdrop-blur-xl shadow-xl">
                            ...
                            <div className="flex gap-2">
                                <input type="text" value={manualInput} onChange={(e) => setManualInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleManualAdd()} placeholder="Enter roll number..." className="flex-1 rounded-xl border border-white/10 bg-black/40 px-5 py-4 text-sm text-white placeholder-white/10 outline-none focus:border-white/30 transition-all" />
                    ...
                    <div className="flex flex-col space-y-4">
                        <div className="flex items-center justify-between px-2">
                            <h2 className="text-xs font-bold uppercase tracking-widest text-white/30">Records ({entries.length})</h2>
                            <button onClick={exportToCSV} disabled={entries.length === 0} className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-white disabled:opacity-20 hover:scale-105 transition-transform">
                                Export .CSV
                            </button>
                        </div>
                                ...
                                <table className="w-full text-left">
                                    ...
                                    <tbody className="divide-y divide-white/[0.04]">
                                        {isLoading ? (
                                            ...
                                            <tr><td colSpan={3} className="py-24 text-center text-white/10 italic text-sm font-light">No entries yet. Ready for scanning.</td></tr>
                                        ) : (
                                            entries.map((entry, index) => (
                                                <tr key={index} className="group transition-colors hover:bg-white/[0.02]">
                                                    ...
                                                    <td className="px-6 py-5 text-right">
                                                        <button onClick={() => entry.id && removeEntry(entry.id)} className="p-2 text-white/5 hover:text-white transition-colors md:opacity-0 group-hover:opacity-100">
